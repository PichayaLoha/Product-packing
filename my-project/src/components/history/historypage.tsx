...
function History_page() {
    ...
    const historyhandle = (e) => {
        ...
        console.log(value);
        navegate('/Historydetail', { state: { message: value } });
                                    ...
                                    <tbody className='text-white text-base'>
                                        {order.map((item, index) => (
                                            <tr key={item.history_id}>
                                                <th>{item.history_id}</th>
                                                <td>{item.history_amount}</td>
                                                {/* <Link></Link> */}
                                                <td>{1}</td>
                                                <td>Bob</td>
                                                <td>{item.history_status}</td>
                                                <td>
                                                    {/* <Link to='/Historydetail'> */}

                                                    <button className='btn btn-sm bg-green-500 border-green-500' data-value={index + 1} onClick={historyhandle}>รายละเอียด</button>
                                                    {/* </Link> */}
